...
# This is a modified version of Pod::Html, written by Tom Christiansen and
# included in the standard distribution of perl.
#
# These modifications include:
#
# - removing the <HTML>, <HEAD> and <STYLE> tags (and closing counterparts)
# - returning the html-ized pod instead of printing it to STDOUT
# - removing the die's for chdirs (this seems to be problematic for
#     unprivileged users)
#
# This modified file should be considered BETA.  Please send bugs and comments to
	...
	if (/^(=.*)/s) {	# is it a pod directive?
		...
		if (/^=(head[1-6])\s+(.*\S)/s) {	# =head[1-6] heading
		    ...
		    warn "$0: $podfile: =item without bullet, number or text"
		       . " in paragraph $paragraph.\n" if !$quiet and (!defined($1) or $1 eq '');
		    process_item( $1 );
		    $after_item = 1;
		} elsif (/^=over\s*(.*)/) {		# =over N
		    process_over();
...
###	    }
    ...
    elsif ($pages{$libpod} =~ /([^:]*\.pod):/ || $pages{$libpod} =~ /([^:]*\.pm):/) {
	    # scan the .pod or .pm file for =item directives
        $pod = $1;
        open(POD, "<$pod") || die "$0: error opening $pod for input: $!\n";
        @poddata = <POD>;
        close(POD);
        clean_data( \@poddata );
...
sub process_text1($$;$$){
	...
	if( $par =~ m{^([^/]+?)/(?!")(.*?)$} ){     # name/ident
        ...
        RESOLVE: {
 	    ...
 	    $url = page_sect( $page, $htmlsection );
            ...
            warn "$0: $podfile: cannot resolve L<$opar> in paragraph $paragraph." unless $quiet;
        }
	...
	if( $lev == 1 ){
	    ...
	    warn "$0: $podfile: undelimited $func<> in paragraph $paragraph." unless $quiet;
	}
    }
    return $res;
}
