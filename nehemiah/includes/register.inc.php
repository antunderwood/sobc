   ...
   if($settings['register_mode']<2)
    {
     ...
     $smarty->assign('subtemplate','register.tpl.inc');
     $template = 'main.tpl';
    }
   else
    {
     ...
     if(empty($errors))
      {
       ...
       // e-mail correct?
       if(!preg_match(EMAIL_PATTERN, $new_user_email)) $errors[] = 'error_email_wrong';
      ...
      if(trim($data['list']) != '')
       {
        $joined_message = my_strtolower($new_user_name.' '.$new_user_email, $lang['charset']);
     ...
     if(empty($errors))
      {
       ...
       if(my_mail($new_user_mailto, $lang['new_user_email_sj'], $lang['new_user_email_txt'])) $smarty->assign('message','registered');
       else $smarty->assign('message','registered_send_error');
       
       $smarty->assign('lang_section','register');
       $smarty->assign('var',htmlspecialchars(stripslashes($new_user_email)));
       $smarty->assign('subnav_location','subnav_register');
       $smarty->assign('subtemplate','info.tpl.inc');
       $template = 'main.tpl';
      }
     else
      {
       ...
       $smarty->assign('subnav_location','subnav_register');
       $smarty->assign('subtemplate','register.tpl.inc');
       $smarty->assign('new_user_name',htmlspecialchars(stripslashes($new_user_name)));
       $smarty->assign('new_user_email',htmlspecialchars(stripslashes($new_user_email)));
       if($settings['terms_of_use_agreement']==1) $smarty->assign("terms_of_use_agreement",true);
       $template = 'main.tpl';
      }
    }
