...
if(isset($_GET['search']))
 {
  ...
  $x_search = stripslashes(preg_replace_callback("#\"(.+?)\"#is", create_function('$string','global $help_pattern; return str_replace(" ",$help_pattern,$string[1]);'), $search));
      ...
      if($row['user_id']>0)
       {
        ...
        else $search_results[$i]['name'] = htmlspecialchars(stripslashes($row['user_name']));
       } 
      else $search_results[$i]['name'] = htmlspecialchars(stripslashes($row['name']));
   
      $search_results[$i]['subject'] = htmlspecialchars(stripslashes($row['subject']));
  ...
  $smarty->assign('search',htmlspecialchars(stripslashes($search)));
...
$smarty->assign('subtemplate','search.tpl.inc');
$template = 'main.tpl';
?>
