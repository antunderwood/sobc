...
if(isset($_GET['edit_user']))
 {
  ...
  $smarty->assign('user_hp',htmlspecialchars(stripslashes($field["user_hp"])));
  $smarty->assign('user_location',htmlspecialchars(stripslashes($field["user_location"])));
  $smarty->assign('profile',htmlspecialchars(stripslashes($field["profile"])));
...
if(isset($_POST['edit_user_submit']))
 {
  ...
  $old_user_name = stripslashes($oun_data['user_name']);
  ...
  */
  ...
  if(empty($errors))
  ...
  else
   {
    ...
    $smarty->assign('edit_user_name', htmlspecialchars(stripslashes($_POST['edit_user_name'])));
    $smarty->assign('edit_user_type', intval($_POST['edit_user_type']));
    $smarty->assign('user_email', htmlspecialchars(stripslashes($_POST['user_email'])));
    $smarty->assign('email_contact', intval($_POST['email_contact']));
    $smarty->assign('user_real_name', htmlspecialchars(stripslashes($_POST['user_real_name'])));
    if(isset($_POST['user_gender'])) $smarty->assign('user_gender', intval($_POST['user_gender']));
    $smarty->assign('user_birthday', htmlspecialchars(stripslashes($_POST['user_birthday'])));
    $smarty->assign('user_hp', htmlspecialchars(stripslashes($_POST['user_hp'])));
    $smarty->assign('user_location', htmlspecialchars(stripslashes($_POST['user_location'])));
    $smarty->assign('profile', htmlspecialchars(stripslashes($_POST['profile'])));
    $smarty->assign('signature', htmlspecialchars(stripslashes($_POST['signature'])));
...
if(isset($_GET['edit_category']))
 {
  ...
  $smarty->assign('edit_category',htmlspecialchars(stripslashes($field['category'])));
...
if (isset($_GET['delete_category']))
 {
  ...
  mysql_free_result($category_result);
  $smarty->assign('category_id',$field['id']);
  $smarty->assign('category_name',stripslashes($field['category']));
...
if (isset($_POST['edit_category_submit']))
 {
  ...
  if(empty($errors))
  ...
  else
   {
    ...
    $smarty->assign('edit_category',htmlspecialchars(stripslashes($_POST['category'])));
...
if(isset($_GET['delete_user']))
 {
  ...
  $selected_users[0]['name'] = htmlspecialchars(stripslashes($user["user_name"]));
  ...
  if (isset($_POST['selected']))
   {
    ...
    for($x=0; $x<$selected_users_count; $x++)
    {
     ...
     $selected_users[$x]['name'] = htmlspecialchars(stripslashes($user["user_name"]));
    }
   $action="delete_users";
   }
  else $action="user";
...
if(isset($_POST['settings_submit']))
 {
  ...
  while(list($key, $val) = each($settings))
   {
    if(isset($_POST[$key])) mysql_query("UPDATE ".$db_settings['settings_table']." SET value='".$_POST[$key]."' WHERE name='".$key."' LIMIT 1", $connid); 
   }
  ...
  if(empty($errors))
   {
    ...
    if(!preg_match(EMAIL_PATTERN, $ar_email)) $errors[] = 'error_email_wrong';
    ...
    if(isset($ar_send_userdata))
     {
      ...
      $new_user_mailto = my_mb_encode_mimeheader($ar_username, CHARSET, "Q")." <".$ar_email.">";
      if($settings['mail_parameter']!='')
       {
        if(!@mail($new_user_mailto, my_mb_encode_mimeheader($lang['admin_reg_user_email_subj'], CHARSET, "Q"), $lang['admin_reg_user_email_text'], $header, $settings['mail_parameter'])) $send_error = '&send_error=true';
       }
      else
       {
        if(!@mail($new_user_mailto, my_mb_encode_mimeheader($lang['admin_reg_user_email_subj'], CHARSET, "Q"), $lang['admin_reg_user_email_text'], $header)) $send_error = '&send_error=true';
       }
     }
...
if(isset($_GET['edit_smiley'])) 
 {
  ...
  $code_1 = stripslashes($data['code_1']);
  $code_2 = stripslashes($data['code_2']);
  $code_3 = stripslashes($data['code_3']);
  $code_4 = stripslashes($data['code_4']);
  $code_5 = stripslashes($data['code_5']);
...
switch($action)
 {
    ...
    while ($line = mysql_fetch_array($result))
     {
      ...
      mysql_free_result($count_result);
      $categories_list[$i]['id'] = stripslashes($line['id']);
      $categories_list[$i]['name'] = htmlspecialchars(stripslashes($line['category']));
      $categories_list[$i]['accession'] = stripslashes($line['accession']);
  ...
  case 'user':
   ...
   if(isset($_GET['new_user'])) $smarty->assign('new_user',htmlspecialchars(stripslashes(urldecode($_GET['new_user']))));
  ...
  case "settings":
   ...
   closedir($handle);
   if(count($templates) > 1) $smarty->assign('more_than_one_template_available',true);
   $smarty->assign('templates',$templates);

   // get available languages:
   $handle=opendir($smarty->config_dir.'/');
   $i=0;
   while ($file = readdir($handle))
    {
     if(strrchr($file, ".")==".lang")
      {
       $language_file[$i]['file'] = $file;
       $language_file[$i]['name'] = ucfirst(str_replace(".lang","",$file));
       $i++;
      }
    }
   closedir($handle);
   if(count($language_file) > 1) $smarty->assign('more_than_one_language_available',true);
    ...
    while($line = mysql_fetch_array($result))
     {
      ...
      $smilies[$i]['file'] = stripslashes($line['file']);
      $smilies[$i]['code_1'] = stripslashes($line['code_1']);
      $smilies[$i]['codes'] = $codes_disp;
      $smilies[$i]['title'] = stripslashes($line['title']); 
      $i++;
     }
    mysql_free_result($result);
   }
   ...
   if(empty($errors))
    { 
     ...
     if(trim($_POST['banned_user_agents']) != '')
      {
       $banned_user_agents_array = explode(',',$_POST['banned_user_agents']);
       ...
       $banned_user_agents = implode(",", $banned_user_agents_array_checked);
      }
   ...
   else
    {
     ...
     $smarty->assign('akismet_key',htmlspecialchars(stripslashes($akismet_key)));
     ...
     $smarty->assign('banned_ips',htmlspecialchars(stripslashes($_POST['banned_ips'])));
     $smarty->assign('banned_user_agents',htmlspecialchars(stripslashes($_POST['banned_user_agents'])));
     $smarty->assign('not_accepted_words',htmlspecialchars(stripslashes($_POST['not_accepted_words'])));
     $smarty->assign('errors',$errors);
     $breadcrumbs[0]['link'] = 'index.php?mode=admin';
     $breadcrumbs[0]['linkname'] = 'subnav_admin_area';
     $smarty->assign('breadcrumbs',$breadcrumbs);
     ...
     while($data= mysql_fetch_array($result))
      {
       ...
       $pages[$i]['title'] = stripslashes($data['title']);
       $pages[$i]['content'] = stripslashes($data['content']);
       $pages[$i]['menu_linkname'] = stripslashes($data['menu_linkname']);
     ...
     if(mysql_num_rows($result)>0)
      {
       $data = mysql_fetch_array($result);
       $smarty->assign('id',$id);
       $smarty->assign('title',htmlspecialchars(stripslashes($data['title'])));
       $smarty->assign('content',htmlspecialchars(stripslashes($data['content'])));
       $smarty->assign('menu_linkname',htmlspecialchars(stripslashes($data['menu_linkname'])));
  ...
  case 'edit_page_submit':
   ...
   if(empty($errors))
   ...
   else
    {
     ...
     $smarty->assign('title',htmlspecialchars(stripslashes($title)));
     $smarty->assign('content',htmlspecialchars(stripslashes($content)));
     $smarty->assign('menu_linkname',htmlspecialchars(stripslashes($menu_linkname)));
   ...
   if(mysql_num_rows($result)>0)
    {
     ...
     $page['title'] = stripslashes($data['title']);
     $smarty->assign('page',$page);
    }
   mysql_free_result($result);
   $breadcrumbs[0]['link'] = 'index.php?mode=admin';
...
$smarty->assign('subtemplate','admin.tpl.inc');
$template = 'main.tpl';
